    ...
    public class TurnSystem : SingleMono<TurnSystem>
    {
        ...
        private void HandleNetworkMessage(WsMessage wsMessage, string payload)
        {
            try
            {
                ...
                {
                    case "GAME_START":
                        var startData = JsonConvert.DeserializeObject<GameStartData>(wsMessage.data);
                        ApplyGameStart(startData);
                        break;
                    case "TURN_START":
                        var turnData = JsonConvert.DeserializeObject<TurnSyncData>(wsMessage.data);
                        ApplyTurnStart(turnData);
                        break;
                    case "PHASE_CHANGE":
                        var phaseData = JsonConvert.DeserializeObject<PhaseSyncData>(wsMessage.data);
                        ApplyPhaseChange(phaseData);
                        break;
                    case "SKIP_SYNC":
                        var skipData = JsonConvert.DeserializeObject<SkipSyncData>(wsMessage.data);
                        SetSkipNextTurn(skipData.playerId, skipData.skip);
                        break;
                    case "PLAYER_DISCONNECTED":
                        var disconnectData = JsonConvert.DeserializeObject<PlayerDisconnectData>(wsMessage.data);
